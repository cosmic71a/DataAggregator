...
public class BeInCryptoScraper implements Extractor {
	...
	public void scrape() {
		// URL of the website
		String baseUrl = "https://beincrypto.com/news/page/";
		...
		for (int i = 1; i <= 7; i++) {
			...
			try {
				...
				//Select <a> elements descend from <h5>
				Elements links = document.select("h5 a");
		...
		writeJsonFile(jsonString, JsonFiles.BEINCRYPTO);
	}
	...
	public BlogObject extractData(String url) {
		try {
			...
			String author = document.select("div.h8--normal-case").text();
	...
	public String extractTime(Document document) {
		String timeText = document.selectFirst("time").text();
	...
	public String extractContent(Document document) {
		Elements paragraphs = document.select("div.entry-content-inner p");
